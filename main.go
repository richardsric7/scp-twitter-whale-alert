...
func SendTwitterMessage(destAmount, fromWallet, toWallet, swapFromAsset, swapToAsset, swapFromAmount, tx string, swap bool) {
	...
	if swap {
		...
		msg = fmt.Sprintf(`🚨🚨🚨 %s swapped %s $%s to %s $%s on wallet %s.
		
		
		Explorer: %s`, fromWallet, sourceWithCommaThousandSep, swapFromAsset, DestAmountWithCommaThousandSep, swapToAsset, toWallet, explorer)
		...
		msg = fmt.Sprintf(`🚨🚨🚨  %s $%s transferred from %s wallet to %s.
		
		
		Explorer: %s`, DestAmountWithCommaThousandSep, swapToAsset, fromWallet, toWallet, explorer)
